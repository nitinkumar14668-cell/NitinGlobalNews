...
import { BookmarkMinus, Loader2, Eye, Calendar, Bookmark, MessageSquare } from 'lucide-react';
...
export function UserProfile() {
  ...
  const [comments, setComments] = useState<(UserComment & { articleTitle: string })[]>([]);
    ...
    const fetchComments = async () => {
      ...
      try {
        ...
        const userComments = snapshot.docs.map(doc => {
          ...
          return null;
        }).filter((c): c is (UserComment & { articleTitle: string }) => c !== null)
    ...
    <main className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8 md:py-12 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
        ...
        <div className="px-6 sm:px-10 pb-8">
          ...
          <div>
            <h1 className="news-serif text-3xl sm:text-4xl font-bold text-slate-900">{user.displayName}</h1>
            ...
            <div className="flex flex-wrap items-center gap-6 mt-6 pt-6 border-t border-slate-100 text-sm text-slate-600">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-slate-700">Joined {creationTime.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</span>
              </div>
              <div className="flex items-center gap-2">
                <Bookmark className="w-5 h-5 text-blue-500" />
                <span className="font-medium text-slate-700">{bookmarks.length} Saved Articles</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div>
              {loading ? (
                ...
                <div className="text-center py-16 bg-slate-50 rounded-xl border border-slate-100 border-dashed flex flex-col items-center">
                  ...
                  <p className="text-slate-500 max-w-sm">Tap the bookmark icon on any article to save it for later reading.</p>
                </div>
              )}
            </div>
          )}
              ...
              {loadingComments ? (
                ...
                <div className="space-y-4">
                  {comments.map(comment => (
                    <div key={comment.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
                        <h4 className="text-sm font-bold text-slate-800 leading-tight">
                          Commented on: <span className="text-blue-600 font-semibold cursor-pointer hover:underline" onClick={() => setSelectedArticle(mockArticles.find(a => a.id === comment.articleId) || null)}>{comment.articleTitle}</span>
                        </h4>
                      ...
                      <p className="text-slate-700 text-sm whitespace-pre-wrap bg-slate-50 p-4 rounded-lg border border-slate-100 border-l-4 border-l-blue-400 italic">"{comment.text}"</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="text-center py-16 bg-slate-50 rounded-xl border border-slate-100 border-dashed flex flex-col items-center">
                  ...
                  <h3 className="text-xl font-bold text-slate-800 mb-2">No comments yet</h3>
                  <p className="text-slate-500 max-w-sm">Share your thoughts on articles and they will appear here.</p>
                </div>
              )}
            </div>
          )}
